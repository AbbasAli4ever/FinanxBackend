...
export class BundleItemDto {
  @IsNotEmpty({ message: 'Product ID is required for each bundle item' })
  @IsUUID('4', { message: 'Product ID must be a valid UUID' })
  productId: string;

  @IsNotEmpty({ message: 'Quantity is required for each bundle item' })
  @IsNumber({}, { message: 'Bundle item quantity must be a number' })
  @Min(0.0001, { message: 'Bundle item quantity must be greater than 0' })
  quantity: number;
}

export class CreateProductDto {
  @IsNotEmpty({ message: 'Product type is required (INVENTORY, NON_INVENTORY, SERVICE, or BUNDLE)' })
  @IsEnum(['INVENTORY', 'NON_INVENTORY', 'SERVICE', 'BUNDLE'], {
    message: 'Product type must be INVENTORY, NON_INVENTORY, SERVICE, or BUNDLE',
  })
  type: string;
  ...
  @IsOptional()
  @IsNumber({}, { message: 'Initial quantity on hand must be a number' })
  @Min(0, { message: 'Initial quantity on hand cannot be negative' })
