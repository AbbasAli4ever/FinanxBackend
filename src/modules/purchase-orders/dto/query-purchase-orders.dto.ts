...
export class QueryPurchaseOrdersDto {
  @IsOptional()
  @IsEnum(
    ['DRAFT', 'SENT', 'PARTIAL', 'RECEIVED', 'CLOSED', 'VOID'],
    {
      message: 'Status must be one of: DRAFT, SENT, PARTIAL, RECEIVED, CLOSED, VOID',
    },
  )
  status?: string;
  ...
  @IsOptional()
  @IsEnum(
    ['poNumber', 'poDate', 'expectedDeliveryDate', 'totalAmount', 'status', 'createdAt'],
    {
      message:
        'Sort by must be one of: poNumber, poDate, expectedDeliveryDate, totalAmount, status, createdAt',
    },
  )
  sortBy?: string;
